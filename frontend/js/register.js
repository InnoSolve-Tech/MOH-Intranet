...
function collectFormData() {
  ...
  supportInputs.forEach((input) => {
    const match = input.name.match(/supportYears\[(\d+)\]\[(\w+)\](?:\[(\w+)\])?/)
    if (match) {
      const index = match[1]
      const field = match[2]
...
async function submitRegistration(formData) {
  ...
  try {
    ...
    formData.contacts.forEach((contact, index) => {
      ...
      submitData.append(`contacts[${index}][alternatePhone]`, contact.alternatePhone || "");
    ...
    formData.supportYears.forEach((supportYear, index) => {
      ...
      submitData.append(`supportYears[${index}][level]`, supportYear.level || "");
      submitData.append(`supportYears[${index}][thematic]`, supportYear.thematic || "");
